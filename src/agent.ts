...
function createRunner(sandboxConfig: SandboxConfig, channelId: string, channelDir: string): AgentRunner {
	...
	// Mutable dynamic context updated before each run
	...
	// Bind extensions with minimal Slack-oriented bindings
	// UI interactions are no-op (no TUI in Slack), errors go to log
		...
		uiContext: {
			...
			notify: (message: string, type?: string) => {
				if (type === "error" || type === "warning") {
					log.logWarning(`Extension ${type}`, message);
				} else {
					log.logInfo(`[extension] ${message}`);
				}
			},
			onTerminalInput: () => () => {},
			setStatus: () => {},
			...
			openEditor: async () => ({ text: "", cancelled: true }),
		} as any,
		onError: (error: { extensionPath: string; event: string; error: string }) => {
			log.logWarning("Extension error", `${error.extensionPath}: [${error.event}] ${error.error}`);
		},
	});
	...
	return {
		async run(
			...
			// Update dynamic context and reload resources (extensions, skills, system prompt)
			dynamicCtx.memory = getMemory(channelDir);
			dynamicCtx.channels = ctx.channels;
			dynamicCtx.users = ctx.users;
			...
			if (runState.totalUsage.cost.total > 0) {
				...
				// Log usage summary to file only, don't post to Slack
